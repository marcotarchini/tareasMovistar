...
var dataLayer,
  ...
  _loQuieroCambioPlan = '.loQuieroCambioPlan',
  _dataTargetModal = '*[data-target="#modal"]',
  _dataTargetModalDisponibilidad = '*[data-target="#modalDisponibilidad"]',
  _dataTargetModalNewFlow = '*[data-target="#modalNewFlow"]',
  _dataTargetModalNewFlowTerminal = '*[data-target="#modalNewFlowTerminal"]',
...
$(function () {
  ...
  function setTipoproducto() {
    $('.js-generic-plan').each(function () {
      var a = $('.tabsNav.active .js-mainBanner:visible .form-group .sendGTM').attr(
          'data-valor-origen',
        ),
        b = $('.tabsNav.active .js-mainBanner:visible .form-group .sendGTM').attr(
          'data-valor-plan',
        ),
        c = $('.tabsNav.active .js-mainBanner:visible .form-group .sendGTM').attr(
          'data-cod-campana',
        );
  ...
  function closeDisponibilidad() {
    //$('.js-formulario-movil-cerrado').hide(),     //oculta luego de las 21hs el boton ctc lead banner
    //$('.js-loquiero-movil-cerrado').show(),     //muestra luego de las 21hs el boton ctc pre-lead banner
    //$('.js-loquiero-mobile').hide(),            //no se esta usando esta clase
    //$('.Llamar-FlotanteDisponibilidad').hide(), //no se esta usando esta clase
  ...
  function setTipoPopUpNocturno(a) {
    switch (a) {
      case 'fija':
        $('.js-modal-disponibilidad-class').addClass('modal-content__nocturno'),
          $('.js-modal-disponibilidad').show(),
          $('.js-modal-disponibilidad').attr('data-disponibilidad-call-center', a),
          $('.modal-content__disponibilidad').removeClass('no-disponibilidad');
        break;
      default:
        $('.js-modal-disponibilidad-class').removeClass('modal-content__nocturno'),
          $('.js-modal-disponibilidad').hide(),
          $('.modal-content__disponibilidad').addClass('no-disponibilidad');
    }
    setHorarioModal(a);
  }
  ...
  function cambiaPestana(a, b) {
    ...
    b !== void 0 && '' !== b && 'undefined' !== b
      ...
      setTipoPopUpNocturno($(a).attr('data-disponibilidad')),
      $('html, body').animate(
        {
          scrollTop: 0,
        },
  ...
  function changeModalNocturno() {
    $('.modalWrapperNocturno .llamame').each(function () {
      $(this).attr('data-valor-url', $('.js-loquiero-movil-cerrado').attr('data-valor-url')),
        ...
        $(this).attr(
          'data-nombre-landing',
          $('j.s-loquiero-movil-cerrado').attr('data-nombre-landing'),
        ),
        $(this).attr('data-valor-plan', $('.js-loquiero-movil-cerrado').attr('data-valor-plan')),
        $(this).attr(
          'data-valor-origen',
          $('.js-loquiero-movil-cerrado').attr('data-valor-origen'),
        ),
  ...
  function sendPin() {
    ...
    $.ajax({
      ...
      .done(function (a) {
        ...
        localStorage.setItem('auth_session', b),
          $('.modal').modal('hide'),
          $('#planIdeal3').modal('show');
      })
      .fail(function (a, b) {
        console.log('Error' + b);
      });
  ...
  function loadDataConfirm() {
    $('.tyc').html(localStorage.getItem('tyc')),
      $('.form-nombre').html(
        localStorage.getItem('product_name') + ' ' + localStorage.getItem('internet'),
      ),
  ...
  function verifyPin() {
    ...
    if (0 < a.trim().length) {
      $.ajax({
        url:
          'https://guia-interactiva.movistararg.com/api/v1/verify_pin/pin/' +
          a +
          '/auth_session/' +
          b,
        method: 'GET',
  ...
  function continuar() {
    ...
    null != a && '' != a && 'empty' != a
      ? sendPin()
      : ($('.modal').modal('hide'), $('#planIdeal2').modal('show'));
  }
  ...
  function abrirLlamame() {
    var a = $(this).attr('data-cod-campana') + ' | ' + $(this).attr('data-valor-plan'),
      b = $(this).text(),
  ...
  function loQuiero() {
    var a = localStorage.getItem('product_name') + ' ' + localStorage.getItem('internet');
    dataLayer.push({
      event: 'LinkClickAction',
      eventCategory: RTD_Planes,
      eventAction: 'Lo_Quiero',
      eventLabel: a,
      eventValue: 1,
      eventNonInteraction: !1,
    }),
      ...
      .done(function (b) {
        ...
        'none' == c
          ...
          : (saveProductData(a, c),
            ...
            $('.cambioPlan .loQuieroCambioPlan').attr(
              'data-valor-plan',
              c.nombre + ' ' + c.internet,
            ),
            ...
            $('.cambioPlan .planesMovistar__paragraph.beneficio_extra1').html(
              'Beneficio Extra: ' + c.beneficio_extra,
            ),
    ...
    localStorage.setItem('product_type', a),
      c
        ...
        : (window.location.href =
            'https://time.movistar.com.ar/dialog/oauth?provider_id=cuoma_01&redirect_uri=' + b);
  }

  function sendForm() {
    $('#formCambioPlan').submit();
  }
  ...
  function validarTel(a, b, c) {
    for (var d = 99, e = '', f = '', g = 0; 0 == g; g++) {
      if ('' == a || isNaN(parseFloat(a)) || !isFinite(a)) {
        b.find('label.error-disclaimer').hide(),
          b.find('label.error').addClass('red'),
          b
            .find('label.error')
            .text(
              'Por favor, ingres\xE1 tu n\xFAmero con c\xF3digo de \xE1rea sin el 0 y sin el 15 (por ejemplo: 11 49999999)',
            ),
          (d = 1),
          (e = 'sin tel'),
          (f =
      ...
      if (10 != a.length) {
        b.find('label.error-disclaimer').hide(),
          ...
          b
            .find('label.error')
            .text('El n\xFAmero debe tener 10 caracteres (por ejemplo: 11 49999999)'),
          (d = 3),
          (e = 'longitud distinta a 10'),
          (f = 'El n\xFAmero debe tener 10 caracteres (por ejemplo: 11 49999999)');
        break;
      }
      if ('1' > a.substring(0, 1) || '3' < a.substring(0, 1) || '111' == a.substring(0, 3)) {
        b.find('label.error-disclaimer').hide(),
          b.find('label.error').addClass('red'),
          b
            .find('label.error')
            .text(
              'N\xFAmero inv\xE1lido. Por favor, ingres\xE1 tu n\xFAmero con c\xF3digo de \xE1rea sin el 0 y sin el 15 (por ejemplo: 11 49999999)',
            ),
          (d = 4),
          (e = 'numero fuera de rango'),
          (f =
            'N\xFAmero inv\xE1lido. Por favor, ingres\xE1 tu n\xFAmero con c\xF3digo de \xE1rea sin el 0 y sin el 15 (por ejemplo: 11 49999999)');
        break;
      }
      var h = '11' + a.substring(2);
      if (
        0 <=
        jQuery.inArray(h, [
          '1149999999',
      ...
      if (!(null == c || 0 > jQuery.inArray(h, c.split('|')))) {
        b.find('label.error-disclaimer').hide(),
          b.find('label.error').addClass('red'),
          b
            .find('label.error')
            .text('N\xFAmero ya registrado. El n\xFAmero fue ingresado previamente.'),
          (d = 6),
          (e = 'numero ya registrado'),
          (f = 'N\xFAmero ya registrado. El n\xFAmero fue ingresado previamente.');
        break;
      }
      d = 0;
      break;
  ...
  function callService(a) {
    ...
    var b,
      ...
      e = $(a).parent().parent();
    b =
      0 == e.find('.cod-urb').length
        ? e.find('.anictc').val()
        : e.find('.cod-urb').val() + e.find('.anictc').val();
    var f = e.find('.orig').val();
    'CERRADO' == estadoCallCenterMovil &&
      ('Movil' == tipo_lp || 'Fija' == tipo_lp) &&
      (f = 'Nocturno|' + f),
        ...
        'CERRADO' === estadoCallCenterFija &&
          (0 >= i
            ? (e.find('label.error-horario').addClass('red'),
              e
                .find('label.error-horario')
                .text('Por favor, seleccion\xE1 un rango horario para contactarte'))
    ...
    if (0 < s.numErr)
      return (
        (r = '|' + f + '|' + g + '|' + b + '|' + o + '|' + h + '|' + c),
        (r = 'formErrorValidacion' + r + '|[' + s.numErr + ']-' + s.descErr),
        dataLayer.push({
          event: 'LinkClickAction',
        ...
        e.find('label.error-label').text(s.userMsg),
        !1
      );
        ...
        jQuery.ajax({
          ...
          success: function () {
            (r = 'formenviado' + r),
              ...
              (u = '\xA1Muchas gracias!'),
              (v =
                'En breve nos estaremos comunicando con vos desde el n\xFAmero 11-2233-0286, de '),
    ...
    return (
      ...
      console.log(
        'idTask=2&param1=' + f + '&param2=' + g + '&param3=' + b + '&param4=' + o + '&param5=' + h,
      ),
      !0
    );
  }
  ...
  function changeNumeroLineaGratuita(a) {
    ...
    '' !== $('.tabsNav.active').attr('data-custom-tel-ctc') &&
      $('.tabsNav.active').attr('data-custom-tel-ctc') !== void 0 &&
      $('.js-numero-linea-gratuita').html(
        $('.tabsNav.active').attr('data-custom-tel-ctc').split(' ').join('-'),
      );
  }
  $('.menu-tab').on('shown.bs.tab', function (a) {
    ...
    $(_modalTerminal).click(function () {
      $('#modal-terminal .modal-title-phone').text($(this).attr('data-title')),
        ...
        $('#modal-terminal .caracteristicas-equipos__item_pantalla p strong').text(
          $(this).attr('data-pantalla'),
        ),
        ...
        $('#modal-terminal .caracteristicas-equipos__item_almacenamiento p strong').text(
          $(this).attr('data-memoria'),
        ),
        $('#modal-terminal .caracteristicas-equipos__item_procesador p strong').text(
          $(this).attr('data-procesador'),
        ),
        $('#modal-terminal .caracteristicas-equipos__item_sistema-operativo p strong').text(
          $(this).attr('data-so'),
        );
    }),
    $(_modalTerminalEquiposLiberados).click(function () {
      $('#modal-terminal-equipos-liberados .js__terminal-modal-titulo').text(
        $(this).attr('data-title'),
      ),
        $('#modal-terminal-equipos-liberados .js__terminal-image-modal').attr(
          'src',
          $(this).attr('data-image'),
        ),
        $('#modal-terminal-equipos-liberados .pantalla p strong').text(
          $(this).attr('data-pantalla'),
        ),
        ...
        $('#modal-terminal-equipos-liberados .almacenamiento p strong').text(
          $(this).attr('data-memoria'),
        ),
        $('#modal-terminal-equipos-liberados .procesador p strong').text(
          $(this).attr('data-procesador'),
        ),
    ...
    $('.moreButton').on('click', function (a) {
      ...
      b.find('.terminos__extendedContent').slideDown(),
        b.find('.lessButton').show(),
        $(this).hide();
    }),
    ...
    $('.close').on('click', function () {
      $('.form-group').find('label.error-disclaimer').show().removeClass('red'),
        $('.form-group').find('.tel_number').val(''),
        $('.form-group')
          .find('label.error-disclaimer')
          .text(
            'Por favor, ingres\xE1 tu n\xFAmero con c\xF3digo de \xE1rea sin el 0 y sin el 15 (por ejemplo: 11 49999999)',
          ),
        $('.form-group').find('label.error').removeClass('red'),
        $('.form-group').find('label.error').text(''),
        reanudarSlider();
    }),
    $(document).on('click', _linkCondiciones, function () {
      sendDataLayer(
        'Link Terminos y Condiciones - Ofertas Avatar - Planes Pospago sin Equipo',
        'Click',
        $(this).find('.text').html(),
      );
    }),
        ...
        if ($(this).attr('data-valor-url') && $(this).attr('id')) {
          ...
          $(_dataTargetModalNocturno).attr('data-cod-campana') &&
            ...
            $(_modalNocturno)
              .find('.llamame')
              .attr(
                'data-disponibilidad-call-center',
                $(this).attr('data-disponibilidad-call-center'),
              );
        setTipoPopUpNocturno($(this).attr('data-disponibilidad-call-center'));
      }
    }),
    $(document).on(
      'click',
      '*[data-target="#modal"],*[data-target="#modalDisponibilidad"]',
      function (a) {
        var b,
        ...
        $(this).attr('data-valor-url') &&
          $(this).attr('id') &&
          ((b = this.attributes['data-valor-url'].value),
          window.open(b, '_blank'),
          $('.modal').modal('hide')),
          $(this).attr('data-cod-campana') &&
            $(_jsBtnLlamemeModal).find('.camp').val($(this).attr('data-cod-campana')),
          $(this).attr('data-valor-origen') &&
            $(_jsBtnLlamemeModal).find('.orig').val($(this).attr('data-valor-origen')),
          $(this).attr('data-valor-plan') &&
            $(_jsBtnLlamemeModal).find('.desc').val($(this).attr('data-valor-plan')),
          $(this).attr('data-disponibilidad-call-center') &&
          ...
          -1 < $(this).attr('data-valor-plan').search('Carrusel Planes')
            ...
            : $('#modalMainTitle').html('Te ayudamos a comprar'),
          ...
          $('.modalWrapper .llamame').each(function () {
            $(this).attr('data-accion', $(e).attr('data-accion')),
              ...
              $(this).attr(
                'data-tipo-call-center-custom',
                $(e).attr('data-tipo-call-center-custom'),
              );
          }),
    ...
    $(document).on('click', _dataTargetModalNewFlow, function () {
      var a = this,
        b = '';
      $(this).attr('data-cod-campana') !== void 0 && (b = $(this).attr('data-cod-campana')),
        null == $(this).attr('data-valor-url') &&
          $('.js-click-cambio').each(function () {
            $(this).attr('data-valor-url', '');
          }),
        -1 < $(this).attr('data-valor-plan').search('Carrusel Planes')
          ? $('#modalMainTitle').html('Te ayudamos a contratar')
        ...
        $('.modalWrapper input#infoModal').val($(this).attr('data-valor-plan')),
        $('.modalWrapper .llamame').each(function () {
          $(this).attr('data-accion', $(a).attr('data-accion')),
            $(this).attr('data-nombre-landing', $(a).attr('data-nombre-landing')),
            $(this).attr('data-valor-plan', $(a).attr('data-valor-plan')),
            $(this).attr('data-valor-origen', $(a).attr('data-valor-origen'));
        }),
        $(this).attr('data-valor-origen') &&
          $(_modalNewFlow).find('.orig').val($(this).attr('data-valor-origen')),
        $('.js-click-cambio').each(function () {
          $(this).attr('data-valor-url', $(a).attr('data-valor-url')),
            $(this).attr('data-accion', $(a).attr('data-accion')),
            $(this).attr('data-nombre-landing', $(a).attr('data-nombre-landing')),
            $(this).attr('data-valor-plan', $(a).attr('data-valor-plan')),
            $(this).attr('data-valor-origen', $(a).attr('data-valor-origen')),
            $(this).attr('data-cod-campana', $(a).attr('data-cod-campana-bifurcador'));
        }),
        $('#terminalTitle').html(''),
        $('#terminalTitle').html($(this).attr('data-modal-title')),
        $('#terminalTitle').show(),
        setTipoPopUpNocturno($(this).attr('data-disponibilidad-call-center')),
        changeModalNocturno(),
        changeNumeroLineaGratuita(b);
    }),
    $(document).on('click', _dataTargetModalNewFlowTerminal, function () {
      var a = this;
      -1 < $(this).attr('data-valor-plan').search('Carrusel Planes')
        ? $('#modalMainTitle').html('Te ayudamos a contratar')
        : $('#modalMainTitle').html('Te ayudamos a comprar'),
    ...
    $(document).on('click', '.js-click-cambio', function () {
      $('.modal').hide();
      var a = $(this).attr('data-cod-campana');
      $(_modal).find('.camp').val(a), sendDataLayer('bifurcador', 'operacion', a);
    }),
    ...
    $(document).on('click', '.js-continuar', function () {
      continuar();
    }),
    $(document).on('click', '.js-abrirLlamame', function () {
      abrirLlamame();
    }),
    $(document).on('click', '.js-reenviar', function () {
      reenviar();
    }),
    $(document).on('click', '.js-ingresarLineaNuevamente', function () {
      $('.modal').modal('hide'), $('#planIdeal2').modal('show');
    }),
    $('#confirm-pin').on('click', function (a) {
      a.preventDefault(),
        dataLayer.push({
          event: 'LinkClickAction',
          eventCategory: RTD_Planes,
          eventAction: 'click',
          eventLabel: 'Ingresa_el_codigo_sms_siguiente',
          eventValue: 1,
          eventNonInteraction: !1,
        }),
        verifyPin();
    }),
    $('#confirm-btn').on('click', function (a) {
      if (
        (a.preventDefault(),
        dataLayer.push({
          event: 'LinkClickAction',
          eventCategory: RTD_Planes,
          eventAction: 'click',
          eventLabel: 'contratar',
          eventValue: 1,
          eventNonInteraction: !1,
        }),
        $('#accept-tyc').is(':checked'))
      ) {
        var b = $('#ani').val();
        'empty' != b && '' != b
          ? sendForm()
          : ($('.modal').modal('hide'), $('#planIdeal2').modal('show'));
      } else alert('Acept\xE1 los t\xE9rminos y condiciones para continuar.');
    }),
    $('#send-ani').on('click', function (a) {
      a.preventDefault(),
        dataLayer.push({
          event: 'LinkClickAction',
          eventCategory: RTD_Planes,
          eventAction: 'click',
          eventLabel: 'Ingresa_tu_telefono_siguiente',
          eventValue: 1,
          eventNonInteraction: !1,
        });
      var b = $('#input-ani').val().trim(),
        c = b.length;
      return '' == b
        ? void alert('Ingres\xE1 el n\xFAmero')
        : void (0 < c && 10 == c && (localStorage.setItem('ani', b), sendPin()),
          0 < c && 12 == c && (localStorage.setItem('ani', b.substring(2)), sendPin()));
    }),
    $('.close').on('click', function () {
    ...
    $(document).on('click', _loQuieroCambioPlan, function () {
      loQuiero();
    }),
    $(document).on('click', '.js-ver-disponibilidad', function () {
      var a = $(this).attr('data-valor-plan'),
        b = $(this).text(),
        c = $(this).attr('data-valor-origen') + ' - ' + nombreLP;
      $(this).hasClass('boton-superior') && (botonClicado.zona = 'Banner Superior'),
        (categoria = c),
    ...
    $(document).on('click', _sendGTMtelefonoHeader, function () {
      sendDataLayer(
        $(this).attr('data-valor-origen'),
        'Click',
        'Compr\xE1 ac\xE1  ' + telefono_contacto,
      );
    }),
    $(document).on('click', _sendClick, function () {
      sendDataLayer(
        $(this).attr('data-valor-origen'),
        'Click',
        'Lo quiero - ' + $(this).attr('data-nombre-landing'),
      );
    }),
    $(document).on('click', _llamame, function () {
      setTipoPopUpNocturno($(this).attr('data-disponibilidad-call-center')), callService(this);
    }),
    $(document).on('click', '.js-click-cambio-banner', function (a) {
      a.preventDefault(), a.stopPropagation();
      var b = $(this).attr('data-cod-campana');
      $('.js-mainBanner:visible .camp').val(b),
        sendDataLayer('bifurcador', 'operacion', b),
        pausarSlider(),
        $('.tabsNav.active').hasClass('carrusel')
          ? callService(
              $('.tabsNav.active .item.active .js-mainBanner:visible .form-group .sendGTM'),
            )
          : callService($('.tabsNav.active .js-mainBanner:visible .form-group .sendGTM'));
    }),
      ...
      $('.js-contact-phone').each(function () {
        ...
        $('#category').on('change', function () {
          ...
          console.log(a),
            'todas' == a
              ? $('.striped').show()
              : ($('#modal-canales table').hide(), $('#' + a).show());
        });
    });
  $(window).load(function () {
    $('#form-disponibilidad').submit(function (a) {
      $('#modalDisponibilidad').modal('hide');
    });
    $('#form-disponibilidad50').submit(function (a) {
      $('#modalDisponibilidadPlanFibra50MB').modal('hide');
    });
  });
});
